...
class Products with ChangeNotifier {
  ...
  Future<void> fetchAndSetProducts([bool filterByUser = false]) async {
    if (authToken != null) {
      ...
      final Uri url = Uri.parse(
          'https://flutter-update-207d3-default-rtdb.firebaseio.com/products.json?auth=$authToken&$filterString');
      try {
        ...
        final Uri favoritesUrl = Uri.parse(
            'https://flutter-update-207d3-default-rtdb.firebaseio.com/userFavorites/$userId.json?auth=$authToken');
  ...
  Future<void> addProduct(Product product) async {
    final Uri url = Uri.parse(
        'https://flutter-update-207d3-default-rtdb.firebaseio.com/products.json?auth=$authToken');
  ...
  Future<void> updateProduct(String id, Product newProduct) async {
    ...
    if (prodIndex >= 0) {
      final Uri url = Uri.parse(
          'https://flutter-update-207d3-default-rtdb.firebaseio.com/products/$id.json?auth=$authToken');
      await retry(
        () => http
            .patch(url,
                body: json.encode({
  ...
  Future<void> toggleFavoriteStatus(String id) async {
    final Uri url = Uri.parse(
        'https://flutter-update-207d3-default-rtdb.firebaseio.com/userFavorites/$userId/$id.json?auth=$authToken');
    const r = RetryOptions(maxAttempts: 20);
    final prodIndex = _items.indexWhere((prod) => prod.id == id);
